...
def normalize(candidate_scores, nondom_scores, extreme_points=None):
    '''
    ...
    if extreme_points is not None:
        augmented_nondom_scores = np.vstack((nondom_scores, extreme_points))
    else:
        augmented_nondom_scores = nondom_scores

    asf = np.max((augmented_nondom_scores-ideal_point) * weights[:,None,:], axis=2)
    I = np.argmin(asf, axis=1)

    new_extreme_points = augmented_nondom_scores[I,:]
    ...
    points.
    '''
    points = new_extreme_points - ideal_point
    print(points)
    ...
    try:
        ...
        intercepts = np.max(nondom_scores, axis=0) - ideal_point
        zeros_in_intercept = intercepts <= 1e-6
        intercepts[zeros_in_intercept] = np.max(candidate_scores, axis=0)[zeros_in_intercept]
        print('Linalg fallback')


    print('Intercepts', intercepts)

    return (candidate_scores - ideal_point)/intercepts, ideal_point, intercepts, new_extreme_points
...
def nsga3(initial_coords, div, extreme_points):
    ...
    normalized_candidate_scores,ideal_point,intercepts,new_extreme_points = normalize(candidate_scores, nondom_scores, extreme_points)
    ...
    return new_population, fronts[0], new_extreme_points


def main():
    init = np.random.random_sample((400, 5)) * (-1)
    start = time.time()
    solution = nsga3(init, 2, None)
    stop = time.time()
    print(stop-start, 's')
